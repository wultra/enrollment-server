...
| `enrollment-server-onboarding.document-verification.microblink.mobile-sdk-configs[i].platform`          |         | Mobile platform. Supported values: `ios`, `android` (case-sensitive). Together with `origin`, this must match the `platform` attribute in the `POST /api/identity/document/init-sdk` request for the `license-key` to be returned. |

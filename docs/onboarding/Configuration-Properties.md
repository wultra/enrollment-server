...
| `enrollment-server-onboarding.document-verification.microblink.mobile-sdk-configs[i].origin`            |         | Microblink Bundle ID / App ID. Together with `platform` forms a composite key for obtaining `license-key`.                              |
| `enrollment-server-onboarding.document-verification.microblink.mobile-sdk-configs[i].platform`          |         | Mobile platform. Example values: `ios`, `android`, `cordova`. Together with `origin` forms a composite key for obtaining `license-key`. |
| `enrollment-server-onboarding.document-verification.microblink.mobile-sdk-configs[i].license-key`       |         | Mobile SDK license key for the given `origin` and `platform` combination.                                                               |

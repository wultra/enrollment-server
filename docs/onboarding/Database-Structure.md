...
| `external_user_id`       | `VARCHAR(256)`  |                                      | External user identifier used by the presence check provider (e.g. iProov). It is `NULL` at the start of the process and set during the presence check phase. |
...
| `error_origin`           | `VARCHAR(256)`  |                                      | Origin of the error (`PROCESS_LIMIT_CHECK`, `USER_REQUEST`).                                                                                                  |
...
| `status`                 | `VARCHAR(32)`  | `NOT NULL`                           | OTP status (`ACTIVE`, `VERIFIED`, `FAILED`).                                            |
...
| `error_origin`           | `VARCHAR(256)` |                                      | Origin of the error (`OTP_VERIFICATION`).                                               |
...
| `error_detail`           | `VARCHAR(256)` |                                      | Detail of error (e.g. information about timeout or exceeded number of failed attempts).                                                                         |
| `error_origin`           | `VARCHAR(256)` |                                      | Origin of the error (`DOCUMENT_VERIFICATION`, `PRESENCE_CHECK`, `CLIENT_EVALUATION`, `PROCESS_LIMIT_CHECK`, `OTP_VERIFICATION`, `FINAL_VALIDATION`, `CLEANUP`). |
| `session_info`           | `TEXT`         |                                      | Attributes related to presence check session in JSON format.                                                                                                    |
...
| `filename`                 | `VARCHAR(256)` | `NOT NULL`                           | Filename specified during upload from mobile client.                                                |
...
| `verification_score`       | `INTEGER`      |                                      | Overall score achieved during document verification and fraud detection (0 - 10).                   |
...
| `error_origin`             | `VARCHAR(256)` |                                      | Origin of the error (`DOCUMENT_VERIFICATION`, `PROCESS_LIMIT_CHECK`).                               |
...
| `error_detail`             | `TEXT`         |                                      | JSON serialized errors which occurred during document processing.    |
| `error_origin`             | `VARCHAR(256)` |                                      | Origin of the error (`DOCUMENT_VERIFICATION`).                       |
...
<!-- begin database table es_onboarding_process_configuration -->
### Onboarding Process Configuration Table

Stores configuration of onboarding processes.
...
|----------------|----------------|------------------------|-----------------------------------------------------|
| `id`           | `BIGINT`       | `NOT NULL PRIMARY KEY` | Autogenerated record identifier (Long).             |
| `process_type` | `VARCHAR(255)` | `NOT NULL`             | Process configuration type, used as an external ID. |
| `config`       | `TEXT`         | `NOT NULL`             | Process configuration as JSON.                      |
...
<!-- begin database table es_selfie -->
### Selfie Table

Temporarily stores selfie images of identity verification.
...
|----------------------------|---------------|--------------------------------------|-----------------------------------------|
| `id`                       | `BIGINT`      | `NOT NULL PRIMARY KEY`               | Autogenerated record identifier (Long). |
| `image`                    | `BYTEA`       |                                      | Raw image data.                         |
| `identity_verification_id` | `VARCHAR(36)` | `NOT NULL`                           | Identity verification identifier.       |
| `timestamp_created`        | `TIMESTAMP`   | `NOT NULL DEFAULT CURRENT_TIMESTAMP` | Timestamp when the selfie was created.  |

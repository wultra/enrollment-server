...
The activation removal endpoint `/pa/v4/activation/remove` now allows authentication using `POSSESSION` factor (1FA). By default, 2FA authentication is used when removing activations, either `POSSESSION_KNOWLEDGE` or `POSSESSION_BIOMETRY`.

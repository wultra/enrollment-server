...
<!-- begin box warning -->
If you customize `logging.pattern.console` and omit `%sa`, structured log data (e.g. action, state, result, IDs, and other useful parameters attached to log messages) will be silently dropped from the console output.
<!-- end -->
...
By default, Enrollment Server logs in a human-readable, plain-text format to the
...
switch to JSON logging by setting the `ENROLLMENT_SERVER_LOGGING` environment
variable to `/opt/logback/conf/es-logback.xml`.

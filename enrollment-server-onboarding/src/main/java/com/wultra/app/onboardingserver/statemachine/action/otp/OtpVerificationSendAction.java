...
public class OtpVerificationSendAction implements Action<EnrollmentState, EnrollmentEvent> {
    ...
    public void execute(StateContext<EnrollmentState, EnrollmentEvent> context) {
        ...
        try {
            if (EnrollmentEvent.OTP_VERIFICATION_RESEND == context.getEvent()) {
                identityVerificationOtpService.resendOtp(identityVerification);
            } else if (EnrollmentEvent.OTP_VERIFICATION_SEND == context.getEvent()) {
                identityVerificationOtpService.sendOtp(identityVerification);
            } else {
                throw new IllegalStateException(
                        String.format("Unexpected event received: %s", context.getEvent())
                );
            }
        } catch (OnboardingProcessException | OnboardingOtpDeliveryException e) {
            logger.warn("Unable to {} OTP, {}",
                    EnrollmentEvent.OTP_VERIFICATION_RESEND == context.getEvent() ? "resend" : "send", ownerId, e
            );
